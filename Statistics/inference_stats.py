...
if __name__ == '__main__':
    ...
    example = 2
    p = 0.01
    stats_type = "seg" # det or seg or cls or prob (modificar thres en StatsModel 74 y prediction to mask)
    ...
    start_epoch = 600  # <= numero de épocas que ya ha entrenado
    color_space = 44 # <= bgr=None, lab=44, yuv=82, hsv=40, hsl=52
    specific_weights = f"real_cls_bias2_v2_lab"
    ...
    # labels = ["b", "y", "o_s", "o_b"]
    label_size = (input_dims[1], input_dims[2], 1)
    background = True
    batch_size = 8
    valid_size = .10
