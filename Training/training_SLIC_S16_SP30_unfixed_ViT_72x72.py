...
from Networks.ViT import SP_unfixed_ViT, SLIC_unfixed_num_patches
...
if __name__ == '__main__':
    set_seeds()
    # Net Variables
    model = "SLIC_unfixed50_ViT_S16_SP30"
    start_epoch = 0
    ...
    tm = TrainingModel(nn=SP_unfixed_ViT(num_classes=6,
    ...
    ts = TrainingStats(model_name=model + id_copy,
                       ...
                       logs_tensorboard=f"{model}/cls/Raabin/batch_{input_dims[0]}/{input_dims[1]}x{input_dims[2]}/unfixed/wo_krgl/AdamW/weight_decay {wd}/lr {lr}/{end_epoch}",
                       start_epoch=start_epoch)
    ...
    for epoch in range(start_epoch + 1, end_epoch + 1):
        ...
        for batch_x, batch_y in train:
            ...
            patches, positions = SLIC_unfixed_num_patches(batch_x.numpy(), region_size=patch_size, ruler=10.,
                                                          iterations=10, projection_dim=projection_dim)
