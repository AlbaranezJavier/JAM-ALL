...
if __name__ == '__main__':
    ...
    ts = TrainingStats(model_name=model + id_copy,
                       ...
                       logs_name=f"{model}_21k1k_retrained/cls/Raabin/batch_{input_dims[0]}/{input_dims[1]}x{input_dims[2]}/AdamW/{lr}/{end_epoch}",
                       start_epoch=start_epoch)
    ...
    for epoch in range(start_epoch + 1, end_epoch + 1):
        ...
        ts.update_values(epoch, is_saved, np.mean(loss_train), np.mean(loss_valid), train_acc, valid_acc, end_time, lr, verbose=1)
